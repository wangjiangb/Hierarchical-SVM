...
public:
	friend class Tree<T>;
    TreeNode():m_firstchild(NULL), m_nextsibling(NULL), m_parent(NULL)
	{
	}
 TreeNode(const string& name, const T &d):m_data(d), m_name(name),m_firstchild(NULL), m_nextsibling(NULL), m_parent(NULL)
	{
	}
    string GetNodeName()
    {
...
public:
	...
	int m_num_features;
	double * m_prob;    
	ClassifierNode():m_data(NULL), m_label(NULL)
	{
	}
	...
	double SvmPredictProb(svm_node* x);//return the probability
